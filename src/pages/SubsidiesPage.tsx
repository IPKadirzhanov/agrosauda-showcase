    ...
    <div className="min-h-screen pt-24 pb-20">
        ...
        <div className="container-main text-center">
          <AnimatedSection>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-sm font-medium mb-6">
              <Sparkles className="w-4 h-4" /> {t.subsidies.aiBadge}
            </div>
            <h1 className="font-display font-bold text-4xl sm:text-5xl mb-4">{t.subsidies.pageTitle}</h1>
            <p className="text-muted-foreground max-w-2xl mx-auto text-lg mb-8">{t.subsidies.pageSubtitle}</p>
          </AnimatedSection>
          <AnimatedSection delay={0.2} className="max-w-xl mx-auto">
            <div className="relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
              <input type="text" placeholder={t.subsidies.searchPlaceholder} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
      ...
      <section className="section-padding bg-muted/20">
        <div className="container-main max-w-2xl">
          <AnimatedSection>
            <h2 className="font-display font-bold text-3xl text-center mb-6">{t.subsidies.askGuide}</h2>
            <AIChatWidget agentType="subsidiya_gid" agentName="СубсидияГид" placeholder={t.subsidies.searchPlaceholder} />
          </AnimatedSection>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-main">
          <AnimatedSection><h2 className="font-display font-bold text-3xl mb-8">{t.subsidies.availablePrograms}</h2></AnimatedSection>
              ...
              <AnimatedSection key={program.id} delay={i * 0.1}>
                <div className="premium-card p-6 rounded-2xl h-full flex flex-col">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-border">
                    <div>
                      <p className="text-xs text-muted-foreground">{t.subsidies.amount}</p>
                      <p className="font-display font-bold text-primary">{program.amount}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">{t.subsidies.deadline}</p>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="font-display font-semibold text-xl">{t.subsidies.noPrograms}</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="section-padding bg-muted/30">
        <div className="container-main max-w-3xl">
          <AnimatedSection><h2 className="font-display font-bold text-3xl text-center mb-8">{t.subsidies.faqTitle}</h2></AnimatedSection>
          {[
            { q: t.subsidies.faq1Q, a: t.subsidies.faq1A },
            { q: t.subsidies.faq2Q, a: t.subsidies.faq2A },
            { q: t.subsidies.faq3Q, a: t.subsidies.faq3A },
          ].map((faq, i) => (
